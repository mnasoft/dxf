...
(defmethod dxf-out-text :before ((x <db-header>) stream)
  (dxf/out/txt:txt-string 0 dxf/sec:*section* stream)
  (dxf/out/txt:txt-string 2 dxf/sec:*section-header* stream))

(defmethod dxf-out-text ((x <db-header>) stream)
  (mapc #'(lambda (el)
	    (when (third el)
	      (dxf/out/txt:txt-string 9 (concatenate 'string "$" (first el)) stream)
	      (dxf/out/txt:txt (second el) (third el) stream)))
	(header-vars x)))

(defmethod dxf-out-text :after ((x <db-header>) stream)
    (dxf/out/txt:txt-string 0 dxf/sec:*endsec* stream))
...
(defmethod dxf-out-text ((x <Acad-Object>) stream)
  (dxf/out/txt:txt-string 0 *Acad-Object-class-marker* stream))

(defmethod dxf-out-text :after ((x <Acad-Object>) stream)
  (let (
	(hdl (handle x))
	(own (owner-id x)))
    (when hdl (dxf/out/txt:txt-hex   5 hdl stream))
    (when own (dxf/out/txt:txt-hex 330 own stream))))
...
(defmethod dxf-out-text ((x <acad-entity>) stream)
  (dxf/out/txt:txt-string 0 *acad-entity-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-entity>) stream)
  (dxf/out/txt:txt-string 100 *acad-entity-subclass-marker* stream)
  (let ((la  (Layer  x))
    ...
    (dxf/out/txt:txt-string 8 la stream)
    (unless (string= "BYLAYER" lt ) (dxf/out/txt:txt 6 lt stream))
    (cond
      ((= dxf/color:+ac-Color-Method-ByLayer+ (dxf/color:color-method cl)))
      ((= dxf/color:+ac-color-method-byblock+ (dxf/color:color-method cl))
       (dxf/out/txt:txt 62 (dxf/color:color-index cl) stream))
      ((= dxf/color:+ac-color-method-byaci+ (dxf/color:color-method cl))
       (dxf/out/txt:txt-int16 62 (dxf/color:color-index cl) stream))
      ((= dxf/color:+ac-color-method-byrgb+ (dxf/color:color-method cl))
       (dxf/out/txt:txt-int16 62  (dxf/color:color-index cl) stream)
       (dxf/out/txt:txt-int32 420 (dxf/color:rgb->true (dxf/color:rgb cl)) stream)))
    (unless (= lts 1.d0) (dxf/out/txt:txt 48 lts stream))
    (unless (= lw -1) (dxf/out/txt:txt-int16  370 lw stream))
    (unless vi  (dxf/out/txt:txt 60 1 stream))))
...
(defmethod dxf-out-text ((x <Acad-Line>) stream)
  (dxf/out/txt:txt-string 0 *Acad-Line-class-marker* stream))

(defmethod dxf-out-text :after ((x <Acad-Line>) stream)
  (dxf/out/txt:txt-string 100 *Acad-Line-subclass-marker* stream)
  (let ((th  (thickness x))
        (p-s (StartPoint x))
	(p-e (EndPoint x))
	(nrm (normal x))
    ...
    (unless (= th 0) (dxf/out/txt:txt-double 39 th stream))
    (dxf/out/txt:txt-point-3d 10 p-s stream)
    (dxf/out/txt:txt-point-3d 11 p-e stream)
    (unless (and (= x-n 0) (= y-n 0) (= z-n 1))
      (dxf/out/txt:txt-point-3d nrm 210 stream))))
...
(defmethod dxf-out-text ((x <acad-point>) stream)
    (dxf/out/txt:txt-string 0 *acad-point-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-point>) stream)
  (dxf/out/txt:txt-string 100 *acad-point-subclass-marker* stream)
  (let ((th  (thickness x))
        (pos (coordinates x))
	(ecs (ecs-angle x))
	(nrm (normal x))
	...
	(z-n (svref (normal x) 2)))
    (unless (= th  0) (dxf/out/txt:txt-double  39 th stream))
    (dxf/out/txt:txt-point-3d 10 pos stream)
    (unless (and (= x-n 0) (= y-n 0) (= z-n 1)) (dxf/out/txt:txt-point-3d 210 nrm stream))
    (unless (= ecs 0) (dxf/out/txt:txt-double 50 ecs stream))))
...
(defmethod dxf-out-text ((x <acad-ray>) stream)
  (dxf/out/txt:txt-string 0 *acad-ray-class-marker* stream))

(defmethod dxf-out-text  :after ((x <acad-ray>) stream)
  (dxf/out/txt:txt-string 100 *acad-ray-subclass-marker* stream)
  (let ((b-p (base-point x))
	(u-d (direction-vector x)))
    (dxf/out/txt:txt-point-3d 10 b-p stream)
...
(defmethod dxf-out-text ((x <acad-xline>) stream)
  (dxf/out/txt:txt-string 0 *acad-xline-class-marker* stream))

(defmethod dxf-out-text  :after ((x <acad-xline>) stream)
  (dxf/out/txt:txt-string 100 *acad-xline-subclass-marker* stream)
  (let ((b-p (base-point x))
	(u-d (direction-vector x)))
    (dxf/out/txt:txt-point-3d 10 b-p stream)
    (dxf/out/txt:txt-point-3d 11 u-d stream)))
...
(defmethod dxf-out-text ((x <Acad-Circle>) stream)
  (dxf/out/txt:txt-string 0 *Acad-Circle-class-marker*  stream))

(defmethod dxf-out-text  :after ((x <Acad-Circle>) stream)
  (dxf/out/txt:txt-string 100 *Acad-Circle-subclass-marker* stream)
  (let ((th (thickness x))
        (p-c (center x))
	(rad (radius x))
	(nrm (normal x))
	...
	(z-n (svref (normal x) 2)))
    (unless (= th 0) (dxf/out/txt:txt-double 39 th stream))
    (dxf/out/txt:txt-point-3d 10 p-c stream)
    (dxf/out/txt:txt-double 40 rad stream)
    (unless (and (= x-n 0) (= y-n 0) (= z-n 1)) (dxf/out/txt:txt-point-3d 210 nrm stream))))
...
(defmethod dxf-out-text ((x <acad-arc>) stream)
  (dxf/out/txt:txt-string 0 *acad-arc-class-marker* stream))
...
(defmethod dxf-out-text :after ((x <acad-arc>) stream)
  ...
  (let ((th (thickness x))
    ...
    (dxf/out/txt:txt-point-3d 10 p-c stream)
    (dxf/out/txt:txt-double 40 rad stream)
    (dxf/out/txt:txt-string 100 *acad-arc-subclass-marker* stream)
    (dxf/out/txt:txt-double 50 s-a stream)
    (dxf/out/txt:txt-double 51 e-a stream)
    (unless (and (= x-n 0) (= y-n 0) (= z-n 1)) (dxf/out/txt:txt-point-3d 210 nrm stream))))

(dxf/out/txt:txt-point-3d 210 (vector 1 2 3)t)
;;;;
...
(defmethod dxf-out-text ((x <acad-text>) stream)
    (dxf/out/txt:txt-string 0 *acad-text-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-text>) stream)
  (dxf/out/txt:txt-string 100 *acad-text-subclass-marker* stream)
  (let ((th (thickness x))
    ...
    (unless (= th 0) (dxf/out/txt:txt-double 39 th stream))
    (dxf/out/txt:txt-point-3d 10 p-p stream)
    (dxf/out/txt:txt-double 40 h stream)
    (dxf/out/txt:txt-string 1 t-s stream)
    (dxf/out/txt:txt-double 50 (* *radian-to-degree* rot) stream)
    (dxf/out/txt:txt-double 41 w-f stream)
    (dxf/out/txt:txt-double 51 (* *radian-to-degree* ob) stream)
    (dxf/out/txt:txt-string 7 st stream)
    (unless (= mir 0) (dxf/out/txt:txt-int16 71 mir stream))
    (unless (= h-j 0) (dxf/out/txt:txt-int16 72 h-j stream))
    (when   (or (/= h-j 0) (/= v-j 0)) (dxf/out/txt:txt-point-3d 11 a-p stream))
    (unless (and (= x-n 0) (= y-n 0) (= z-n 1)) (dxf/out/txt:txt-point-3d 210 nrm stream))
    (dxf/out/txt:txt-string 100 *acad-text-subclass-marker* stream)
    (unless (= v-j 0) (dxf/out/txt:txt-int16 73 v-j stream))))
...
(defmethod dxf-out-text ((x <acad-ellipse>) stream)
  (dxf/out/txt:txt-string 0 *acad-ellipse-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-ellipse>) stream)
  (dxf/out/txt:txt-string 100 *acad-ellipse-subclass-marker* stream)
  (let ((p-c   (center x))
	...
	(e-p (end-parameter   x)))
    (dxf/out/txt:txt-point-3d 10 p-c stream)
    (dxf/out/txt:txt-point-3d 11 p-ma stream)
    (unless (and (= u-n-x 0) (= u-n-y 0) (= u-n-z 1)) (dxf/out/txt:txt-point-3d 210 u-n stream))
    (dxf/out/txt:txt-double 40 r-r stream)
    (dxf/out/txt:txt-double 41 s-p stream)
    (dxf/out/txt:txt-double 42 e-p stream)))
...
(defmethod dxf-out-text ((x <acad-layer>) stream)
  (dxf/out/txt:txt-string 0 *acad-layer-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-layer>) stream)
  (dxf/out/txt:txt-string 100 *acad-symboltablerecord-subclass-marker* stream)
  (dxf/out/txt:txt-string 100 *acad-layer-subclass-marker* stream)
  (let ((name            (name            x))
	(freeze          (freeze          x))
...
;;; (when lock            (setf c-70 (dpb 1 (byte 1 4) c-70)))
    ...
    (dxf/out/txt:txt-string  2 name   stream)
    (dxf/out/txt:txt-int16  70 c-70   stream)
    (cond
      ((= 0   (first cl)) (dxf/out/txt:txt 62 (first cl) stream))
      ((and (< 0 (first cl) 256) (null (second cl)))
       (dxf/out/txt:txt-int16 62 (* (first cl) layeron) stream))
      ((and (< 0 (first cl) 256) (second cl))
       (dxf/out/txt:txt-int16  62 (* (first cl) layeron) stream)
       (dxf/out/txt:txt-int32 420 (dxf/color:rgb->true (second cl)) stream)))
    (dxf/out/txt:txt         6 l-ltype   stream)
    (dxf/out/txt:txt       290 plottable stream)
    (dxf/out/txt:txt       370 l-lweight stream)
    (dxf/out/txt:txt       390 l-pstyle  stream)
    (dxf/out/txt:txt       347 l-mat     stream)
;;; (dxf/out/txt:txt       348 l-vstyle  stream)
    ))
...
(defmethod dxf-out-text ((x <acad-layers>) stream)
  (dxf/out/txt:txt-string 2 *acad-layer-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-layers>) stream)
  (dxf/out/txt:txt-string 100 *acad-symboltable-subclass-marker* stream)
  (let ((a-count (a-count x))
	(items   (items   x)))
    (dxf/out/txt:txt-int16  70 a-count   stream)
...
(defmethod dxf-out-text :after ((x <acad-document>) stream)
  (block section-header
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "HEADER"  stream)
    (mapc #'(lambda (header)
	      (mapc #'(lambda (el) (dxf/out/txt:txt (first el) (second el) stream)) header))
	  (sec-header x))
    (dxf/out/txt:txt 0 "ENDSEC" stream))
  (block section-classes
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "CLASSES" stream)
    (mapc #'(lambda (class)
	      (mapc #'(lambda (el) (dxf/out/txt:txt (first el) (second el) stream)) class))
	  (sec-classes x))
    (dxf/out/txt:txt 0 "ENDSEC" stream))
  (block section-tables
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "TABLES" stream)
;;;
    (dxf/out/txt:txt 0 "ENDSEC" stream)
    )
  (block section-blocks
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "BLOCKS" stream)
;;;
    (dxf/out/txt:txt 0 "ENDSEC" stream)
    )
  (block section-entities
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "ENTITIES" stream)
;;;
    (dxf/out/txt:txt 0 "ENDSEC" stream)
    )
  (block section-objects
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "OBJECTS" stream)
;;;
    (dxf/out/txt:txt 0 "ENDSEC" stream)
    )
    (block section-objects
    (dxf/out/txt:txt 0 "SECTION" stream)
    (dxf/out/txt:txt 2 "ACDSDATA" stream)
;;;
    (dxf/out/txt:txt 0 "ENDSEC" stream)
    )
    
    (dxf/out/txt:txt 0 "EOF" stream)
  )
...
(defmethod dxf-out-text ((x <acad-linetype>) stream)
  (dxf/out/txt:txt-string 0 *acad-linetype-class-marker* stream))

(defmethod dxf-out-text :after ((x <acad-linetype>) stream)
  (dxf/out/txt:txt-string 100 *acad-symboltablerecord-subclass-marker* stream)
  (dxf/out/txt:txt-string 100 *acad-linetype-subclass-marker* stream)
  (let ((name            (name            x))
	(description     (description     x))
	(pairs           (pairs           x)))
    (dxf/out/txt:txt-string  2 name   stream)
    (dxf/out/txt:txt-int16  70 (cadr (assoc 70 pairs))   stream)
    (dxf/out/txt:txt-string  3 description stream)
    (mapcar
     #'(lambda (el)
	 (dxf/out/txt:txt (car el) (cadr el) t))
...
(defmethod dxf-out-text ((x <db-symbol-tbl>) stream)
  (dxf/out/txt:txt-string 0 *symbol-tbl-class-marker* stream)
    (let ((st-name (Object-Name x)))
    (dxf/out/txt:txt-string 2 st-name stream)))

(defmethod dxf-out-text :after ((x <db-symbol-tbl>) stream)
  (dxf/out/txt:txt-string 100 *db-symbol-tr-subclass-marker* stream)
  (let ((st-flag  (symbol-tbl-flag x))
	(st-items (reverse (symbol-tbl-items x))))
    (dxf/out/txt:txt-int16 70 st-flag stream)
    (mapc #'(lambda (el) (dxf-out-text el stream)) st-items)
    (dxf/out/txt:txt-string 0 *end-tab* stream)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defmethod dxf-out-text ((x <db-symbol-tr>) stream)
  (dxf/out/txt:txt-string 0 *db-symbol-tr-class-marker* stream))
...
(defmethod dxf-out-text ((x <db-block-tr>) stream)
  (dxf/out/txt:txt-string 0 *db-block-tr-class-marker* stream))

(defmethod dxf-out-text :after ((x <db-block-tr>) stream)
  (dxf/out/txt:txt-string 100 *db-block-tr-subclass-marker* stream)
  (let ((st-name (symbol-tr-name x))
    ...
    (dxf/out/txt:txt-string 2 st-name stream)
    (when lay (dxf/out/txt:txt-hex 340 lay stream))
    (dxf/out/txt:txt-int16 70 st-flag stream)
    (dxf/out/txt:txt-int16 280 e-ty stream)
    (dxf/out/txt:txt-int16 281 s-ty stream)
    (when b-p (dxf/out/txt:txt-hex 310 b-p stream))))
