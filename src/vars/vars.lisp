...
(defparameter *$CEPSNID* '("$CEPSNID" 390 )
  "Plotstyle handle of new objects; if CEPSNTYPE is 3, then this value
  indicates the handle")

(defparameter *$CEPSNTYPE* '("$CEPSNTYPE" 380 )
  "Plot style type of new objects: 0 = Plot style by layer; 1 = Plot
  style by block; 2 = Plot style by dictionary default; 3 = Plot style
  by object ID/handle")
...
(defparameter *$CSHADOW* '("$CSHADOW" 280 )
  "Shadow mode for a 3D object: 0 = Casts and receives shadows 1 =
  Casts shadows 2 = Receives shadows 3 = Ignores shadows")
...
0 = Creates exploded dimensions; there is no association between
elements of the dimension, and the lines, arcs, arrowheads, and text
of a dimension are drawn as separate objects;

1 = Creates non-associative dimension objects; the elements of the
dimension are formed into a single object, and if the definition point
on the object moves,then the dimension value is updated;

2 = Creates associative dimension objects; the elements of the
dimension are formed into a single object and one or more definition
points of the dimension are coupled with association points on
geometric objects")
...
(defparameter *$DIMATFIT* '("$DIMATFIT" 70 )
  "Controls dimension text and arrow placement when space is not
...
(defparameter *$OBSCOLOR* '("$OBSCOLOR" 70 )
  "Specifies the color of obscured lines. An obscured line is a hidden
line made visible by changing its color and linetype and is visible
only when the HIDE or SHADEMODE command is used. The OBSCUREDCOLOR
setting is visible only if the OBSCUREDLTYPE is turned ON by setting
it to a value other than 0.

0 and 256 = Entity color 1-255 = An AutoCAD color index (ACI)")

(defparameter *$OBSLTYPE* '("$OBSLTYPE" 280 )
  "Specifies the linetype of obscured lines. Obscured linetypes are
independent of zoom level, unlike regular AutoCAD linetypes. Value 0
turns off display of obscured lines and is the default. 
...
(defparameter *$PROJECTNAME* '("$PROJECTNAME" 1 )
  " Assigns a project name to the current drawing. Used when an
external reference or image is not found on its original path. The
project name points to a section in the registry that can contain one
or more search paths for each project name defined.  Project names and
their search directories are created from the Files tab of the Options
dialog box")
...
(defparameter *$PUCSORTHOREF* '("$PUCSORTHOREF" 2 )
  ...
  this is the name of the UCS that the orthographic UCS is relative
  to. If blank, UCS is relative to WORLD")


(defparameter *$PUCSORTHOVIEW* '("$PUCSORTHOVIEW" 70 )
  "Orthographic view type of paper space UCS: 0 = UCS is not
  orthographic;
...
(defparameter *$SORTENTS* '("$SORTENTS" 280 )
  " Controls the object sorting methods; accessible from the Options
...
(defparameter *$VERSIONGUID* '("$VERSIONGUID" 2 )
  "Uniquely identifies a particular version of a drawing. Updated when
  the drawing is modified")

(defparameter *$VISRETAIN* '("$VISRETAIN" 70 )
  "0 = Don't retain xref-dependent visibility settings; 1 = Retain
  xref-dependent visibility settings")

(defparameter *$WORLDVIEW* '("$WORLDVIEW" 70 )
...
(defparameter *$XEDIT* '("$XEDIT" 290 )
  "Controls whether the current drawing can be edited in-place when
  being referenced by another drawing. 1 = Can use in-place reference
  editing")
